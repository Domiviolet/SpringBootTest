...
public class AsymmetricEncrypt {
    ...
    @Test
    public void testGenerateKeyToAppointedPaths() throws NoSuchAlgorithmException, IOException {
        ...
        //把生成的私钥公钥放在指定的路径 "aPrivatePath", "aPublicPath"中 (注意如果该路径已经已经存在,就不再创建)
        FileUtils.writeStringToFile(new File(aPrivatePath), privateKeyStr, StandardCharsets.UTF_8);
        FileUtils.writeStringToFile(new File(aPublicPath), publicKeyStr, StandardCharsets.UTF_8);

        System.out.println("END");//表示结束
