...
public class DigitalSignature {
    ...
    public void test1() throws NoSuchAlgorithmException, IOException, InvalidKeySpecException, SignatureException, InvalidKeyException {
        //调用获取私钥对象的方法,获取私钥对象
        PrivateKey privateKey = this.readPrivateKeyObject("a.pri", "RSA");
        //调用获取公钥对象的方法,获取公钥对象
        PublicKey publicKey = this.readPublicKeyObject("a.pub", "RSA");
        //调用获取数字签名的方法,获取数字签名(密文)  //注意这里的算法SHA256,必须跟 with+上面密钥加密的算法
        String signatureData = this.getSignature("123", "SHA256withRSA", privateKey);
        System.out.println(signatureData);

        //进行校验
        boolean b = this.verifySignature("123", "SHA256withRSA",publicKey, signatureData);
